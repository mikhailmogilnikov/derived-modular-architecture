---
title: "Astro"
description: "DMA с composition root в pages/ и frontmatter-импортами."
icon: "Sparkles"
---

Пример для **Astro 5**: слои под `src/`, composition root в `src/pages/`, layout и providers в `src/app/`.
...
| `src/app/` | Layout, providers, seed — **не** роуты |
...
1. Импортируйте `*/public/*` и stage-0 модули
...
Порты невидимы для статического графа — используйте для seed/cycle break, не для обхода promotion. Подробнее — [Склейка модулей](/guides/cross-module-wiring).

## State patterns

1. **Colocated store** — `catalog.store.ts`, импорт relative из `public/`
2. **Promoted service** — `services/cart/public/cart.ts` (catalog + checkout)
3. **Provider port** — `ports.ts` + bind в `app/`
...
Biome plugin — **best-effort** (GritQL). Нет полного path resolution и graph cycles. **Всегда** `dma check` в CI.
