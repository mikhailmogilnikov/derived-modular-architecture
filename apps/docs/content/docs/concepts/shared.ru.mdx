...
`ui` и `api` — наименее стабильные жители shared; первые кандидаты на вынос в пакет (stage 4).

## Граница shared vs services

Shared — переносимый код. Services — продуктовый сценарий с inbound от модулей. Если сомневаетесь — [Services vs Shared](/deep-dive/services-vs-shared).
...
| `shared-candidate` (doctor) | Файл импортируют 2+ модуля — пора подумать о lift |
...
<Cards>
  ...
  <Card title="Services vs Shared" description="Deep dive о границе" href="/deep-dive/services-vs-shared" />
</Cards>
