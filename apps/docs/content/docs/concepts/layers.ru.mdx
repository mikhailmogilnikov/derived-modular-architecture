...
Папки `services/` **может не быть** в начале проекта. Пока ни один модуль не стал общим для двух потребителей, services не нужен — правила, которые на неё ссылаются, просто не срабатывают.
