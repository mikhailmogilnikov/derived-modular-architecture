...
Unit tests stay in the app package (`apps/web/src/...`). Workspace package tests — inside the package. `dma check` analyzes **one graph per root** (app or discovered package); graphs are not merged across the monorepo. See [Monorepo](/guides/monorepo).
